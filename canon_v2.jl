...
using LinearAlgebra
using QuantumClifford  # Provides Stabilizer and Tableau types

# Set a constant for the minimum number of blocks to launch.
const MIN_BLOCKS = 36
...
function custom_gpu_linear_copy(arr::Array{T}) where T
    ...
    # Allocate a destination device array
    d_arr_dest = CUDA.zeros(T, N)
    threads = 256  # a multiple of 32
    # Force a minimum number of blocks even if N is small.
    blocks = MIN_BLOCKS  
...
function gpu_swap_columns_and_phases!(d_mat, d_phases, col1::Int, col2::Int)
    N = size(d_mat, 1)
    threads = 256  # must be multiple of 32
...
function update_kernel!(d_mat, d_phases, current_col::Int32, pivot_row::Int32, bit::UInt64,
    ...
    # Allocate dynamic shared memory for pivot column:
    s_pivot = @cuDynamicSharedMem(UInt64, total_blocks)
    tid = threadIdx().x
    # Each thread loads elements from the pivot column into shared memory.
...
###########################################################################
# 3. GPU Canonicalization Routine (Full Version)
###########################################################################
function gpu_canonicalize!(gt::GpuTableau, phases::Bool)
    ...
    # Second pass: process the Z–part.
    for j in Int32(1):nqubits
...
###########################################################################
function benchmark_canonicalize_gpu(nqubits::Int, phases::Bool)
    println("Benchmarking phases = $phases GPU-resident canonicalization for nqubits = $nqubits...")
    cpu_stab = QuantumClifford.random_stabilizer(nqubits)
    gpu_stab = cpu_to_gpu_stabilizer(cpu_stab)
    t = @belapsed begin
        local s = gpu_stabilizer_copy($gpu_stab)
        canonicalize_gpu!(s; phases=$phases)
        CUDA.synchronize()
    end
    println("GPU-resident canonicalization time: $(t * 1000) ms")
end
...
benchmark_canonicalize_gpu(6000, true)
